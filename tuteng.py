...
def get_config():
    try:
        config_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'Tutengapi.json')
...
class TutengBaseNode:
    def __init__(self):
        # Default URLs, can be overridden by inputs
        self.midjourney_api_url = {
            "turbo mode": "https://yunwu.ai/mj-turbo",
            "fast mode": "https://yunwu.ai/mj-fast",
            "relax mode": "https://yunwu.ai/mj-relax"
        }
    ...
    def update_base_url(self, base_url):
        """Updates the API URLs based on a new base URL."""
        if base_url and base_url.strip():
            self.midjourney_api_url = {
                "turbo mode": f"{base_url.strip()}/mj-turbo",
                "fast mode": f"{base_url.strip()}/mj-fast",
                "relax mode": f"{base_url.strip()}/mj-relax"
            }
...
class Tuteng_upload(TutengBaseNode):
    """
    Tuteng_upload node
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "base_api_url": ("STRING", {"default": "https://yunwu.ai"}),
            }
        }
    ...
    async def upload_image_to_midjourney(self, image):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                upload_url = f"{self.midjourney_api_url[self.speed]}/mj/submit/upload-discord-images"
                async with session.post(upload_url, headers=self.get_headers(), json=payload, timeout=self.timeout) as response:
                    if response.status == 200:
                        data = await response.json()
    ...
    def upload_image(self, image, api_key="", base_api_url=""):
        try:
            ...
            self.update_base_url(base_api_url)
                
            if not self.api_key:
                raise Exception("API key not found. Please provide an API key in the node or in Tutengapi.json")
...
class Tuteng_Mj(TutengBaseNode):
    ...
    def process_input(self, speed, text, text_en="", image=None, model_version=None, ar=None, no=None, c=None, s=None, iw=None, r=None, sw=None, cw=None, sv=None, video=False, tile=False, seed=0, cref="none", oref="none", sref="none", positive="", api_key="", base_api_url=""):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
...
class Tuteng_Mju(TutengBaseNode):
    ...
    def run(self, taskId, U1=False, U2=False, U3=False, U4=False, base_api_url=""):
        self.update_base_url(base_api_url)
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
...
class Tuteng_Mjv(TutengBaseNode):
    class MidjourneyError(Exception):
        pass
    ...
    def run(self, taskId, upsample_v6_2x_subtle=False, upsample_v6_2x_creative=False, costume_zoom=False, zoom="", pan_left=False, pan_right=False, pan_up=False, pan_down=False, base_api_url=""):
        self.update_base_url(base_api_url)
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
...
class Tuteng_mjstyle:
            ...
            for file in files:
                if file.endswith(".json"):
                    with open(os.path.join(root, file), 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        for d in data:
                            Tuteng_mjstyle.styleAll[d['name']] = d
    ...
    def run(self, extra_pnginfo, unique_id, styles_type, positive="", negative=""):
        ...
        for val in values:
            if "prompt" in Tuteng_mjstyle.styleAll[val]:
                if "{positive}" in Tuteng_mjstyle.styleAll[val]["prompt"] and not has_prompt:
                    style_positive = Tuteng_mjstyle.styleAll[val]["prompt"].format(positive=positive)
                    has_prompt = True
                else:
                    style_positive += ", " + Tuteng_mjstyle.styleAll[val]["prompt"].replace(", {positive}", "").replace("{positive}", "")
            if "negative_prompt" in Tuteng_mjstyle.styleAll[val]:
                style_negative += ', ' + Tuteng_mjstyle.styleAll[val]['negative_prompt'] if negative else Tuteng_mjstyle.styleAll[val]['negative_prompt']
...
class Tuteng_kling_text2video:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "api_key": ("STRING", {"default": ""}),
                "api_url": ("STRING", {"default": "https://yunwu.ai/kling/v1/videos/text2video"}),
    ...
    def generate_video(self, prompt, model_name, imagination, aspect_ratio, mode, duration, num_videos, 
                  negative_prompt="", camera="none", camera_value=0, seed=0, api_key="", api_url="https://yunwu.ai/kling/v1/videos/text2video"):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
        ...
        if not self.api_key:
            error_response = {"task_status": "failed", "task_status_msg": "API key not found in Tutengapi.json"}
...
class Tuteng_kling_image2video:
    @classmethod 
    def INPUT_TYPES(cls):
        return {
            "required": {
            ...
            "optional": {
                "image_tail": ("IMAGE",),
                "api_key": ("STRING", {"default": ""}),
                "api_url": ("STRING", {"default": "https://yunwu.ai/kling/v1/videos/image2video"}),
                "camera": (["none", "horizontal", "vertical", "zoom", "vertical_shake", "horizontal_shake", 
                          "rotate", "master_down_zoom", "master_zoom_up", "master_right_rotate_zoom", 
                          "master_left_rotate_zoom"], {"default": "none"}),
                "camera_value": ("FLOAT", {"default": 0, "min": -10, "max": 10, "step": 0.1}),      
    ...
    def generate_video(self, image, prompt, model_name, imagination, aspect_ratio, mode, duration, 
                  num_videos, negative_prompt="", camera="none", camera_value=0, seed=0, image_tail=None, api_key="", api_url="https://yunwu.ai/kling/v1/videos/image2video"):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
...
class Tuteng_video_extend:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "api_url": ("STRING", {"default": "https://yunwu.ai/kling/v1/videos/video-extend"}),
            }
        }
    ...
    def extend_video(self, video_id, prompt="", api_url="https://yunwu.ai/kling/v1/videos/video-extend"):
        if not self.api_key:
            error_response = {"task_status": "failed", "task_status_msg": "API key not found in Tutengapi.json"}
            return ("", "", json.dumps(error_response))
...
class Tuteng_lip_sync:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            ...
            "optional": {
                "api_url": ("STRING", {"default": "https://yunwu.ai/kling/v1/videos/lip-sync"}),
    ...
    def process_lip_sync(self, video_id, task_id, mode, text, voice_language, zh_voice, en_voice, voice_speed, seed=0,
                    video_url="", audio_type="file", audio_file="", audio_url="", api_url="https://yunwu.ai/kling/v1/videos/lip-sync"):
...
class Tuteng_kling_videoPreview:
    @classmethod
    def INPUT_TYPES(s):
        return {"required":{
            "video":("VIDEO",),
...
class TutengGeminiAPI:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "api_key": ("STRING", {"default": ""}),
                "api_url": ("STRING", {"default": "https://yunwu.ai/v1/chat/completions"}),
                "object_image": ("IMAGE",),  
                "subject_image": ("IMAGE",),
                "scene_image": ("IMAGE",),
            }
    ...
    def process(self, prompt, model, resolution, num_images, temperature, top_p, seed, timeout=120, 
                object_image=None, subject_image=None, scene_image=None, api_key="", api_url="https://yunwu.ai/v1/chat/completions"):
        # Update API key if provided
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
...
class TutengJimengApi:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "api_key": ("STRING", {"default": ""}),
                "upload_api_url": ("STRING", {"default": "https://yunwu.ai/v1/files"}),
                "chat_api_url": ("STRING", {"default": "https://yunwu.ai/v1/chat/completions"}),
                "cv_process_api_url": ("STRING", {"default": "https://yunwu.ai/volcv/v1?Action=CVProcess&Version=2022-08-31"}),
    ...
    def upload_image(self, image_tensor, upload_api_url):
    ...
    def generate_image(self, prompt, scale=2.5, seed=-1, width=1328, height=1328, use_pre_llm=False, 
                      ...
                      upload_api_url="https://yunwu.ai/v1/files", 
                      chat_api_url="https://yunwu.ai/v1/chat/completions",
                      cv_process_api_url="https://yunwu.ai/volcv/v1?Action=CVProcess&Version=2022-08-31"):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
        ...
        try:
            if not self.api_key:
                error_message = "API key not found in Tutengapi.json"
                print(error_message)
            ...
            if image is not None:
                ...
                uploaded_image_url = self.upload_image(image, upload_api_url)
...
class TutengJimengVideoApi:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "api_key": ("STRING", {"default": ""}),
                "upload_api_url": ("STRING", {"default": "https://yunwu.ai/v1/files"}),
                "submit_api_url": ("STRING", {"default": "https://yunwu.ai/jimeng/submit/videos"}),
                "fetch_api_url_base": ("STRING", {"default": "https://yunwu.ai/jimeng/fetch"}),
                "image": ("IMAGE",),
                "seed": ("INT", {"default": 0, "min": 0, "max": 2147483647})
            }
        }
    ...
    def generate_video(self, prompt, duration, aspect_ratio, cfg_scale, api_key="", image=None, seed=0,
                       upload_api_url="https://yunwu.ai/v1/files",
                       submit_api_url="https://yunwu.ai/jimeng/submit/videos",
                       fetch_api_url_base="https://yunwu.ai/jimeng/fetch"):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
        ...
        if not self.api_key:
            error_response = {"code": "error", "message": "API key not found in Tutengapi.json"}
            return ("", "", json.dumps(error_response), "")
        ...
        try:
            ...
            if image is not None:
                pbar.update_absolute(20)
                image_url = self.upload_image(image, upload_api_url)
                if image_url:
...
class TutengSeededit:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "api_key": ("STRING", {"default": ""}),
                "api_url": ("STRING", {"default": "https://yunwu.ai/volcv/v1?Action=CVProcess&Version=2022-08-31"}),
                "add_logo": ("BOOLEAN", {"default": False}),
                "logo_position": (["右下角", "左下角", "左上角", "右上角"], {"default": "右下角"}),
                "logo_language": (["中文", "英文"], {"default": "中文"}),
                "logo_text": ("STRING", {"default": "", "multiline": False}),
    ...
    def edit_image(self, image, prompt, scale=0.5, seed=-1, add_logo=False, logo_position="右下角", 
                   logo_language="中文", logo_text="", api_key="", api_url="https://yunwu.ai/volcv/v1?Action=CVProcess&Version=2022-08-31"):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
...
class Tuteng_gpt_image_1_edit:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "mask": ("MASK",),
                "api_key": ("STRING", {"default": ""}),
                "api_url": ("STRING", {"default": "https://yunwu.ai/v1/images/edits"}),
    ...
    def format_conversation_history(self):
        ...
        if not Tuteng_gpt_image_1_edit._conversation_history:
            return ""
        formatted_history = ""
        for entry in Tuteng_gpt_image_1_edit._conversation_history:
    ...
    def edit_image(self, image, prompt, model="gpt-image-1", n=1, quality="auto", 
              seed=0, mask=None, api_key="", size="auto", clear_chats=True, api_url="https://yunwu.ai/v1/images/edits"):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
        ...
        if not clear_chats and Tuteng_gpt_image_1_edit._last_edited_image is not None:
            if original_batch_size > 1:
                last_batch_size = Tuteng_gpt_image_1_edit._last_edited_image.shape[0]
                last_image_first = Tuteng_gpt_image_1_edit._last_edited_image[0:1]
                ...
                image = Tuteng_gpt_image_1_edit._last_edited_image
                use_saved_image = True

        if clear_chats:
            Tuteng_gpt_image_1_edit._conversation_history = []
        ...
        try:
            ...
            if edited_images:
                ...
                Tuteng_gpt_image_1_edit._conversation_history.append({
                    "user": f"Edit image with prompt: {prompt}",
                    "ai": f"Generated edited image with {model}"
                })
 
                Tuteng_gpt_image_1_edit._last_edited_image = combined_tensor
...
class Tuteng_gpt_image_1:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "api_key": ("STRING", {"default": ""}),
                "api_url": ("STRING", {"default": "https://yunwu.ai/v1/images/generations"}),
                "model": ("STRING", {"default": "gpt-image-1"}),
                "n": ("INT", {"default": 1, "min": 1, "max": 10}),
                "quality": (["auto", "high", "medium", "low"], {"default": "auto"}),
                "size": (["auto", "1024x1024", "1536x1024", "1024x1536"], {"default": "auto"}),
    ...
    def generate_image(self, prompt, model="gpt-image-1", n=1, quality="auto", 
                ...
                moderation="auto", seed=0, api_key="", api_url="https://yunwu.ai/v1/images/generations"):
...
class TutengChatGPTApi:
    ...
    def __init__(self):
        ...
        # This will now be set by the input, but we can keep a default here.
    ...
    def process(self, prompt, model, clear_chats=True, files=None, image_url="", images=None, temperature=0.7, 
               ...
               image_download_timeout=100, api_key="", api_url="https://yunwu.ai/v1/chat/completions"):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
        ...
        # Set the API URL from the input
        ...
        try:
            ...
            if not clear_chats and TutengChatGPTApi._last_generated_image_urls:
                prev_image_url = TutengChatGPTApi._last_generated_image_urls.split('\n')[0].strip()
                if prev_image_url:
            ...
            if image_urls:
                TutengChatGPTApi._last_generated_image_urls = image_urls_string
...
class Tuteng_Flux_Kontext:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "upload_api_url": ("STRING", {"default": "https://yunwu.ai/v1/files"}),
                "generate_api_url": ("STRING", {"default": "https://yunwu.ai/v1/images/generations"}),
    ...
    def generate_image(self, prompt, input_image=None, model="flux-kontext-pro", 
                  ...
                  upload_api_url="https://yunwu.ai/v1/files",
                  generate_api_url="https://yunwu.ai/v1/images/generations"):
        if apikey.strip():
            self.api_key = apikey
            config = get_config()
        ...
        try:
            ...
            if input_image is not None:
                ...
                for i in range(batch_size):
                    ...
                    image_url = self.upload_image(single_image, upload_api_url)
                    if image_url:
            ...
            elif not clear_image and Tuteng_Flux_Kontext._last_image_url:
                final_prompt = f"{Tuteng_Flux_Kontext._last_image_url} {prompt}"
            ...
            if generated_tensors:
                ...
                if image_urls:
                    Tuteng_Flux_Kontext._last_image_url = image_urls[0]
...
class Tuteng_Flux_Kontext_Edit:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
            ...
            "optional": {
                ...
                "model": (["flux-kontext-dev", "flux-kontext-pro", "flux-kontext-max"], {"default": "flux-kontext-pro"}),
                "apikey": ("STRING", {"default": ""}),
                "edit_api_url": ("STRING", {"default": "https://yunwu.ai/v1/images/edits"}),
                "generate_api_url": ("STRING", {"default": "https://yunwu.ai/v1/images/generations"}),
    ...
    def generate_image(self, prompt, image=None, model="flux-kontext-pro", 
                  ...
                  seed=-1, 
                  edit_api_url="https://yunwu.ai/v1/images/edits", 
                  generate_api_url="https://yunwu.ai/v1/images/generations"):
        if apikey.strip():
            self.api_key = apikey
            config = get_config()
...
class Tuteng_Flux_Kontext_bfl:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "api_key": ("STRING", {"default": ""}),
                "api_url_base": ("STRING", {"default": "https://yunwu.ai/bfl/v1"}),
                "input_image": ("IMAGE",),
                "seed": ("INT", {"default": -1, "min": -1, "max": 2147483647}),
                "aspect_ratio": (["21:9", "16:9", "4:3", "3:2", "1:1", "2:3", "3:4", "9:16", "9:21"], 
                         {"default": "1:1"}),
    ...
    def generate_image(self, prompt, model="flux-kontext-pro", input_image=None, 
                      ...
                      api_url_base="https://yunwu.ai/bfl/v1"):
        if api_key.strip():
            self.api_key = api_key
            config = get_config()
            config['api_key'] = api_key
        ...
        api_endpoint = f"{api_url_base}/{model}"
            ...
            while attempts < max_attempts:
                ...
                try:
                    result_response = requests.get(
                        f"{api_url_base}/get_result?id={task_id}",
                        headers=self.get_headers(),
                        timeout=self.timeout
                    )
...
class Tuteng_Google_Veo3:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "apikey": ("STRING", {"default": ""}),
                "submit_api_url": ("STRING", {"default": "https://yunwu.ai/google/v1/models/veo/videos"}),
                "task_status_api_url_base": ("STRING", {"default": "https://yunwu.ai/google/v1/tasks"}),
                "image": ("IMAGE",),  
            }
        }
    ...
    def generate_video(self, prompt, model="veo3", enhance_prompt=False, apikey="", image=None,
                       submit_api_url="https://yunwu.ai/google/v1/models/veo/videos",
                       task_status_api_url_base="https://yunwu.ai/google/v1/tasks"):
        if apikey.strip():
            self.api_key = apikey
            config = get_config()
            config['api_key'] = apikey
        ...
        try:
            ...
            while attempts < max_attempts:
                ...
                try:
                    status_response = requests.get(
                        f"{task_status_api_url_base}/{task_id}",
                        headers=self.get_headers(),
                        timeout=self.timeout
                    )
...
NODE_CLASS_MAPPINGS = {
    "Tuteng_Mj": Tuteng_Mj,
    "Tuteng_mjstyle": Tuteng_mjstyle,
    "Tuteng_upload": Tuteng_upload,
    "Tuteng_Mju": Tuteng_Mju,
    "Tuteng_Mjv": Tuteng_Mjv,  
    "Tuteng_kling_text2video": Tuteng_kling_text2video,
    "Tuteng_kling_image2video": Tuteng_kling_image2video,
    "Tuteng_video_extend": Tuteng_video_extend,
    "Tuteng_lip_sync": Tuteng_lip_sync,
    "Tuteng_kling_videoPreview": Tuteng_kling_videoPreview,  
    "TutengGeminiAPI": TutengGeminiAPI,
    "TutengSeededit": TutengSeededit,
    "TutengChatGPTApi": TutengChatGPTApi,
    "TutengJimengApi": TutengJimengApi, 
    "Tuteng_gpt_image_1_edit": Tuteng_gpt_image_1_edit,
    "Tuteng_gpt_image_1": Tuteng_gpt_image_1,
    "TutengJimengVideoApi": TutengJimengVideoApi,
    "Tuteng_Flux_Kontext": Tuteng_Flux_Kontext,
    "Tuteng_Flux_Kontext_Edit": Tuteng_Flux_Kontext_Edit,
    "Tuteng_Flux_Kontext_bfl": Tuteng_Flux_Kontext_bfl,
    "Tuteng_Google_Veo3": Tuteng_Google_Veo3,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "Tuteng_Mj": "Tuteng Mj", 
    "Tuteng_mjstyle": "Tuteng Mj Style",
    "Tuteng_upload": "Tuteng Upload",
    "Tuteng_Mju": "Tuteng Mj Upscale",
    "Tuteng_Mjv": "Tuteng Mj Vary/Zoom",  
    "Tuteng_kling_text2video": "Tuteng Kling Text2Video",
    "Tuteng_kling_image2video": "Tuteng Kling Image2Video",
    "Tuteng_video_extend": "Tuteng Kling Video Extend",
    "Tuteng_lip_sync": "Tuteng Kling Lip Sync",
    "Tuteng_kling_videoPreview": "Tuteng Kling Video Preview",  
    "TutengGeminiAPI": "Tuteng Gemini API",
    "TutengSeededit": "Tuteng Doubao SeedEdit",
    "TutengChatGPTApi": "Tuteng ChatGPT API",
    "TutengJimengApi": "Tuteng Jimeng API", 
    "Tuteng_gpt_image_1_edit": "Tuteng GPT-Image-1 Edit",
    "Tuteng_gpt_image_1": "Tuteng GPT-Image-1 Generate", 
    "TutengJimengVideoApi": "Tuteng Jimeng Video API",
    "Tuteng_Flux_Kontext": "Tuteng Flux Kontext",
    "Tuteng_Flux_Kontext_Edit": "Tuteng Flux Kontext Edit",
    "Tuteng_Flux_Kontext_bfl": "Tuteng Flux Kontext BFL",
    "Tuteng_Google_Veo3": "Tuteng Google Veo3",
}
